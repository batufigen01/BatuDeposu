...
function handleStart(url: URL): Response {
  const config: OAuthConfig = {
    ...
    redirectUri: `${url.origin}/oauth-instagram/callback`,
    scope: 'user_profile,user_media',
  };

  if (!config.clientId || !config.clientSecret) {
    return createErrorResponse('Instagram OAuth not configured', 500);
  }
...
async function handleCallback(url: URL, req: Request): Promise<Response> {
  ...
  return new Response(null, {
    ...
    headers: {
      ...corsHeaders,
      Location: `${frontendUrl}?oauth=success&platform=instagram`,
    },
  });
}
