...
function handleStart(url: URL): Response {
  const config: OAuthConfig = {
    ...
    redirectUri: `${url.origin}/oauth-facebook/callback`,
    scope: 'pages_show_list,pages_read_engagement,pages_manage_posts',
  };

  if (!config.clientId || !config.clientSecret) {
    return createErrorResponse('Facebook OAuth not configured', 500);
  }
...
async function handleCallback(url: URL, req: Request): Promise<Response> {
  ...
  return new Response(null, {
    ...
    headers: {
      ...corsHeaders,
      Location: `${frontendUrl}?oauth=success&platform=facebook`,
    },
  });
}
